...
The meat and potatoes for all machine learning algorithms is having good and lots of training samples. This is even more true for deep networks. I downloaded the Udacity's [training set](https://d17h27t6h515a5.cloudfront.net/topher/2016/December/584f6edd_data/data.zip). The downloaded data.zip consists of a driving_log.csv file and an IMG folder full of recorded images from the car's cameras. Both, the csv file and the images are generated by the simulator.
...
- steering: The steering angle for the center image. A value between -1s and 1.
...
- Left and right camera images: Left and right camera images are used additionaly to the center image. Therefore, I added a constant offset, 0.25, to the angle for the left camera image and subtracted this offset from the angle for the right camera. The code for this operations is in the ```generator(...)``` function.
```python
  ...
  # Left image
